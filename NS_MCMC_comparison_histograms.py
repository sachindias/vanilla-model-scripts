...
#PLOTS HISTOGRAMS & CENTILES PER PARAMETER (& FOR MCMC/NS)
def plot_histograms(data, colour, label):
    #PLOTS HISTOGRAM
    plt.hist(data, bins = 20, histtype="step", color = colour, linewidth = 2, density = True, label = label, alpha = 0.9)
    
    #CACLULATES & PLOTS 16TH, 50TH & 84TH CENTILES
    q_16 = np.quantile(data, 0.16)
    q_50 = np.quantile(data, 0.5)
    q_84 =np.quantile(data, 0.84)
    
    plt.axvline(x = q_16, linestyle = "--", color = colour)
    plt.axvline(x = q_84, linestyle = "--", color = colour)
    plt.axvline(x = q_50, linestyle = "-", color = colour, linewidth = 2)
        
def plotter(params, MCMC_values, NS_values, save_filename = ""):
    plt.figure(figsize=(20, 16*len(params)))
    number_plots = len(params)

    #MAKE THE PLOTS
    for n in range(len(params)):
        plt.subplot(number_plots*4,4,n+1)
        
        plot_timer = datetime.datetime.now()
        plot_histograms(MCMC_values[n], "tab:blue", "MCMC")
        plot_histograms(NS_values[n], "tab:red", "NS")
        print("PARAM %s: %s, PLOT TIME: %s" %(n+1, params[n], datetime.datetime.now() - plot_timer))
