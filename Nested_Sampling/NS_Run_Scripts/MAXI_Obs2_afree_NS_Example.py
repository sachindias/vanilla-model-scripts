...
xspec.AllData("1:1 rev3533_cor_pileup_PN_spectrum_grp.fits 2:2 March_A_OM_Nustar_grp_spectrum.pha 3:3 March_B_OM_Nustar_grp_spectrum.pha")

xspec.AllData.ignore(f"1:1 **-0.8 1.75-2.35 10.0-**")
xspec.AllData.ignore(f"2:2 **-5.0 70.0-**")
xspec.AllData.ignore(f"3:3 **-3.0 70.0-**")
...
model.relxillCp.gamma.values = (1.59663,0.01,1.2,1.2,3,3)
...
model.relxillCp.norm.values = (0.000950156,0.01,1.42085e-09,1.42085e-09,0.142085,0.142085)
...
#UNTIE AND SET UNIFORM PRIORS FOR CONSTANT FACTORS IN EACH DATA GROUP
for data_grp in range(2, xspec.AllData.nGroups+1):

    xspec.AllModels(data_grp)(1).untie()
    xspec.AllModels(data_grp)(1).values = (1, 0.01, 0.5, 0.5, 3, 3)
    priors.append(bxa.create_uniform_prior_for(xspec.AllModels(data_grp), xspec.AllModels(data_grp).constant.factor))
